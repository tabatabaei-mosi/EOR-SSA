...
class SSA_ANN:
    ...
    def __init__(
        ...
        self.X_train, self.y_train, self.X_test, self.y_test = dataset[
            "x_train"], dataset["y_train"], dataset["x_test"], dataset["y_test"]
        ...
        self.charts_path = path_check(
            file_path="Results/Charts", path_return=True)
    ...
    def create_model(self):
        ...
        for i in range(len(self.n_hidden_nodes)):
            ...
            if i == 0:
                ...
                model.add(
                    Dense(self.n_hidden_nodes[i], activation=self.activation[i]))
        ...
        self.problem_size = self.n_dims = np.sum(
            [np.size(w) for w in self.model.get_weights()])
    ...
    def decode_solution(self, solution):
        ...
        weight_sizes = [(w.shape, np.size(w))
